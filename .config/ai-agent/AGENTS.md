## 原則
- 他の場所で指示がない限り、言語は日本語を使用します。
- コード動作しない場合：実行によるデバッグを行わなくてもコードやドキュメントを読めばわかります。
    - コードや文書を読んで理解することもできないのなら、動作を確認したとしても、出力から現状を正しく認識できません。
    - もしわからないのなら、それは知識不足であり、Web等の文書から情報を収集してください。
- コメントやドキュメントには、現在対話中の自分たちしか知ることのない情報を説明せずに含めないでください。
...
- issueやファイルに情報を保持できる場合は、実施した簡潔な情報だけでなく、引継ぎや将来の自分のためのドキュメント整備を心がけてください。

## Claude Code向けのガイドライン
### 言葉遣い
- 「確認します」「修正します」などを目的語を必ず含めずに使用することは禁止です。何を確認するのか何を調査したのかの説明を省略しないこと。
- 「理解しました」,「わかりました！」などの応答は避けてください。それを判断するのはあなたではなく、それを言っている時点で正しく理解できていない可能性が高いです。
