## 原則
- 基本的に大枠の方針以外は、ユーザが全て指示するのではなく、自律的に考えて作業します。ただし、情報はユーザに細かく共有する必要があり、どう行動するかをユーザに提案して、それの許可や助言をもらうというスタイルで進めると効率が良いです。あなたが判断してよいですが、共有していない情報で進めると手戻りが発生しやすくなったり、損害が発生する可能性があります。
...
- DDDではなくなりますが、実装パターンに応じて分離されていることは許容範囲です。
- 要件が定義されてないプロジェクトでは、テストは品質の保証になりません。モデルだけ綺麗でも仕様と関係がないものが出来上がるし、保証するべき点が細かくなりすぎて運用負荷が高くなります。逆に仕様ベースのテストだけでも設計が不十分になりやすいです。そのようなテストやlintは価値は低く、仕様と設計の明確化が必要になります。
### 非推奨
...
<ClaudeInstructions>
### コンテキストの節約
Readするファイルが増える代わりに、サブエージェントで作業すると余計な情報を知ることなく作業ができます。
サブエージェントへの指示は具体的に行いましょう。抽象的に指示をすると、網羅的な調査に時間がかかる上、出力が膨大になります。
...
- 注意されたときに、「XXXはしません」等と表現するのも非推奨です。スコープが広い表現であるからです。

### 避けるべき操作
- 想像でドキュメントやPRのUse Caseやよくある質問、Test Planを追加しないようにしましょう。誰にとっての質問か、誰に向けたテストプランなのかを明記してください。
